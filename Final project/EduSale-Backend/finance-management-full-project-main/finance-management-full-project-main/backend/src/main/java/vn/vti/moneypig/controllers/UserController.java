...
//        this.otpService = otpService;
    ...
    public ResponseEntity<?> insert(@RequestParam String token, @RequestBody User user)
    {
        ...
        if(isAuthenticated){
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject(201, null,"Authenticated not exist"));
        }
    ...
    public ResponseEntity<?> findAll(@RequestParam String token){
        ...
        if(token.isBlank()){
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject(201, null,"toke is not exist"));
        }
        token = "Bearer " + token;
        ...
        if(isAuthenticated){
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject(201, null,"isauthenticated not exist"));
        }
    }
    @GetMapping("/findById")
